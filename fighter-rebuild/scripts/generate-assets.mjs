...
function drawStageLayerSky() {
  ...
  drawPixelText(bmp, "MORTALCODEX", 188, 26, "#f2f5ff", 4);
  return bmp;
}
...
function drawContactSheet(manifest) {
  ...
  drawPixelText(bmp, "MORTALCODEX ORIGINAL ASSET CONTACT", 32, 24, "#f3f5f4", 2);
  let x = 36;
...
async function writeStageSkylineOnly() {
  await writePng("/assets/stages/byte-boardroom/skyline.png", drawStageLayerSky());
  await assertPng("/assets/stages/byte-boardroom/skyline.png", STAGE.width, STAGE.height);
  console.log("Updated stage byte-boardroom skyline layer.");
}

async function main() {
  if (process.argv.includes("--stage-skyline-only")) {
    await writeStageSkylineOnly();
    return;
  }
...
function notesFor(manifest) {
  return `# Mortal Codex Asset Generation Notes
